// Isolated store for the Simulator tab.
// NEVER read or write from Fixtures tab or Tournament tab.
...
export const useSimulatorStore = create<SimulatorStoreState>()(
  persist(
    ...
    {
      name: 'fitbolpix-simulator-store',
      storage: createJSONStorage(() => AsyncStorage),
    },
  ),
);

// ─── Selectors (same API as useMatchStore) ────────────────────────────────────

export function selectSimStandings(
  standings: Record<string, TeamStanding>,
  group: string,
): TeamStanding[] {
...
export function selectSimResult(
  results: Record<string, MatchResult>,
  fixtureId: string,
): MatchResult | null {
