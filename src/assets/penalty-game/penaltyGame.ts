...
// ═══════════════════════════════════════════════════════════════════
// LAYOUT CONSTANTS (all proportional to W × H)
// ═══════════════════════════════════════════════════════════════════
var W=414, H=736;
...
var CROWD_H    = 40;
var AD_BOARD_H = 24;
var AD_BOARD_Y = CROWD_H;                // 40
var SCORE_TOP  = CROWD_H + AD_BOARD_H;   // 64
var SCORE_H    = 60;                      // compact scoreboard

// Goal — wide landscape proportions (~3:1)
var GOAL_W   = Math.round(W * 0.58);     // 240px wide
var GOAL_H   = Math.round(GOAL_W / 3);   // 80px tall
var GOAL_X   = Math.round((W - GOAL_W) / 2);
var GOAL_Y   = SCORE_TOP + SCORE_H + 8;  // 132 — just below scoreboard
var POST_W   = 5;
var CBAR_H   = 5;

var INNER_X  = GOAL_X + POST_W;
var INNER_Y  = GOAL_Y + CBAR_H;
var INNER_W  = GOAL_W - POST_W * 2;
var INNER_H  = GOAL_H - CBAR_H;
var ZONE_W   = INNER_W / 3;
var ZONE_H   = INNER_H / 3;

var GOAL_BOTTOM = GOAL_Y + GOAL_H;       // 212

// Penalty box & 6-yard box — anchored to goal line (GOAL_BOTTOM)
var PBW = Math.round(W * 0.72);
var PBH = Math.round(H * 0.22);
var PBX = Math.round((W - PBW) / 2);
var PBY = GOAL_BOTTOM;                    // top of pen box = goal line
var SBW = Math.round(W * 0.38);
var SBH = Math.round(H * 0.08);
var SBX = Math.round((W - SBW) / 2);
var SBY = PBY;
var PB_BOTTOM = PBY + PBH;               // ~374

// Penalty spot — inside pen box, ~75% down from goal line
var SPOT_X   = Math.round(W / 2);
var SPOT_Y   = PBY + Math.round(PBH * 0.72);  // ~328

// D-arc — centered on penalty spot, radius reaches just to PB bottom
var DARC_R   = 73;                               // standard penalty arc proportion

// Goalkeeper — on the goal line, inside goal frame
var GK_X     = Math.round(W / 2);
var GK_Y     = GOAL_BOTTOM - 10;               // just inside goal

// Kicker and controls — below penalty box
var KICKER_X = Math.round(W / 2);
var KICKER_Y = PB_BOTTOM + 56;                 // ~430

var CTRL_Y   = KICKER_Y + 38;                  // ~468
...
// ═══════════════════════════════════════════════════════════════════
// SHOT RESOLUTION — per-technique power/accuracy rules
// ═══════════════════════════════════════════════════════════════════
...
function resolveUserShot(zone, power, accuracy, technique, gkDive, gkRating) {
  ...
  if (technique === 'regular') {
    if (accuracy < 15) return 'miss';
    if (power > 95) return 'miss';
    if (power > 90) return Math.random() < 0.7 ? 'miss' : 'saved';
    if (power < 60 && covers) return 'saved';
    var saveProb = covers ? 0.25 + gkRating * 0.005 : 0.03;
    return Math.random() < saveProb ? 'saved' : 'goal';
  }

  if (technique === 'power') {
    if (accuracy < 15) return 'miss';
    if (power >= 100) return 'miss';
    if (power > 98) return Math.random() < 0.8 ? 'miss' : 'saved';
    if (power < 80 && covers) return 'saved';
    var saveProb2 = covers ? 0.15 + gkRating * 0.003 : 0.02;
    return Math.random() < saveProb2 ? 'saved' : 'goal';
  }

  if (technique === 'panenka') {
    if (power > 50) return 'miss';
    if (power > 45) return Math.random() < 0.7 ? 'miss' : 'saved';
    if (power < 25) return 'saved';
    if (gkDive !== 'center') return 'goal';
    return Math.random() < 0.65 ? 'saved' : 'goal';
  }
...
// ═══════════════════════════════════════════════════════════════════
// COMMENTARY — English, with technique-specific lines
// ═══════════════════════════════════════════════════════════════════
var LINES = {
  goal: [
    'GOOOAL! 🎉','TOP BINS! Unstoppable!','NET BUSTER! 💥',
    'SIUUUU! Right in the corner!','The keeper had no chance!','WORLD CLASS finish! 🌟'
  ],
  saved: [
    'SAVED! The keeper guessed right!','What a stop! 🧤','No way through!',
    'The keeper read it perfectly!','Incredible reflexes!'
  ],
  miss: [
    'Oh no... Over the bar!','POST! So unlucky! 😬','Ballooned it! What a miss!',
    'The pressure got to them!','Way over! Into the stands!'
  ],
  goal_panenka: [
    'CHEEKY PANENKA! 😏 Audacious!','Ice in the veins! Panenka!','What composure! Right down the middle!'
  ],
  goal_power: [
    'THUNDERBOLT! 💥 Unstoppable!','Absolute ROCKET! The net is still shaking!'
  ],
  saved_panenka: [
    'GK read it! The Panenka failed!','Too obvious! Keeper stayed central!'
  ]
};
function pickLine(outcome, technique) {
  var specificKey = outcome + '_' + technique;
  if (LINES[specificKey] && Math.random() < 0.7) {
    var pool = LINES[specificKey];
...
var GameScene = new Phaser.Class({
  ...
  create: function() {
    ...
    // ── Ball trail (power shot) ─────────────────────────────────────
    this.ballTrail     = [];
    this.ballInFlight  = false;
    this.flightTech    = 'regular';
  ...
  // ─────────────────────────────────────────────────────────────────
  // CROWD — v0.8 stadium fans with individual sprites (depth 0–1)
  // ─────────────────────────────────────────────────────────────────
  _buildCrowd: function() {
    ...
    for (var row = 0; row < ROWS; row++) {
      var rowY = 4 + row * (CROWD_H / ROWS);
      for (var fi = 0; fi < FIGS; fi++) {
        // Deterministic jitter
        var jx = ((fi * 7 + row * 13) % 9 - 4) * 0.8;
        var jy = ((fi * 3 + row * 11) % 7 - 3) * 0.4;

        // HOME fans (left half)
        var hx = 4 + fi * ((W / 2 - 8) / FIGS) + jx;
        var hy = rowY + jy;
        var hColor = fi % 3 === 0 ? homeKit : lighten(homeKit, 30);
        var hg = this.add.graphics().setDepth(2);
        // Body (shirt)
        hg.fillStyle(hColor);
        ...
        // AWAY fans (right half)
        var ax = W / 2 + 4 + fi * ((W / 2 - 8) / FIGS) + jx;
        var ay = rowY + jy;
        var aColor = fi % 3 === 0 ? awayKit : lighten(awayKit, 30);
        var ag = this.add.graphics().setDepth(2);
        ag.fillStyle(aColor);
  ...
  // ─────────────────────────────────────────────────────────────────
  // PITCH — grass fills full canvas, correct markings
  // ─────────────────────────────────────────────────────────────────
  _buildPitch: function() {
    var g = this.add.graphics().setDepth(0);

    // Alternating grass stripes — full canvas width, full height
    var sw = Math.ceil(W / 8);
    for (var i = 0; i < 9; i++) {
      g.fillStyle(i % 2 === 0 ? 0x2d7a3a : 0x266b32);
      g.fillRect(i * sw, 0, sw, H);
    }

    // Goal line — at GOAL_BOTTOM (top edge of penalty box)
    g.lineStyle(2, 0xffffff, 0.5);
    g.beginPath(); g.moveTo(0, GOAL_BOTTOM); g.lineTo(W, GOAL_BOTTOM); g.strokePath();
    ...
    g.fillCircle(SPOT_X, SPOT_Y, 3);

    // D-arc — only the portion OUTSIDE (below) the penalty box
    // Compute the angle where the arc intersects PB_BOTTOM
    var dy = PB_BOTTOM - SPOT_Y;
    var startAngle, endAngle;
    if (dy < DARC_R) {
      var halfAngle = Math.acos(dy / DARC_R);
      // We want the bottom portion: from (PI/2 - halfAngle) to (PI/2 + halfAngle)
      // In Phaser's coordinate system (y-down), 0=right, PI/2=down
    ...
    g.beginPath();
    g.arc(SPOT_X, SPOT_Y, DARC_R, startAngle, endAngle, false);
    g.strokePath();
  },

  // ─────────────────────────────────────────────────────────────────
  // AD BOARDS — top and bottom, 6 colored panels each
  // ─────────────────────────────────────────────────────────────────
  _buildAdBoards: function() {
    ...
    for (var i = 0; i < 6; i++) {
      g.fillStyle(panelColors[i]);
      g.fillRect(i * pw, ay, pw, AD_BOARD_H);
      if (i > 0) {
        g.fillStyle(0xffffff, 0.3);
        g.fillRect(i * pw, ay, 1, AD_BOARD_H);
      }
      this.add.text(i * pw + pw / 2, ay + AD_BOARD_H / 2, panelTexts[i], {
    ...
    g.fillRect(0, ay, W, 1);
    g.fillRect(0, ay + AD_BOARD_H - 1, W, 1);
  },

  // ─────────────────────────────────────────────────────────────────
  // NET (depth 5) — diagonal crosshatch
  // ─────────────────────────────────────────────────────────────────
  _buildNet: function() {
  ...
  _drawNet: function(scaleX) {
    ...
    for (var d = -nh; d < nw; d += step) {
      var x1 = Math.max(0, d), y1 = Math.max(0, -d);
      var x2 = Math.min(nw, d + nh), y2 = Math.min(nh, -d + nw);
      g.beginPath(); g.moveTo(nx + x1, ny + y1); g.lineTo(nx + x2, ny + y2); g.strokePath();
    }
    // Top-right to bottom-left diagonals
    for (var d2 = 0; d2 < nw + nh; d2 += step) {
      var x1b = Math.min(nw, d2), y1b = Math.max(0, d2 - nw);
      var x2b = Math.max(0, d2 - nh), y2b = Math.min(nh, d2);
      g.beginPath(); g.moveTo(nx + x1b, ny + y1b); g.lineTo(nx + x2b, ny + y2b); g.strokePath();
    }
    // Depth shading (darker at back)
    g.fillStyle(0x000000, 0.15);
    g.fillRect(nx, ny, nw, nh * 0.25);
  },

  // ─────────────────────────────────────────────────────────────────
  // GOAL POSTS (depth 6) — thin white posts with perspective
  // ─────────────────────────────────────────────────────────────────
  _buildPosts: function() {
    var g = this.add.graphics().setDepth(6);

    // 3-D shadow behind posts
    g.fillStyle(0x333333, 0.6);
    g.fillRect(GOAL_X + POST_W + 2, GOAL_Y + 2, POST_W, GOAL_H);
    g.fillRect(GOAL_X + GOAL_W - POST_W + 2, GOAL_Y + 2, POST_W, GOAL_H);
    g.fillRect(GOAL_X + 2, GOAL_Y + CBAR_H + 2, GOAL_W, CBAR_H);
    ...
    // Perspective side posts (angled lines going back from top corners)
    g.lineStyle(2, 0xcccccc, 0.5);
    // Left perspective
    g.beginPath();
    g.moveTo(GOAL_X, GOAL_Y);
    g.lineTo(GOAL_X - 6, GOAL_Y - 10);
    g.strokePath();
    // Right perspective
    g.beginPath();
    g.moveTo(GOAL_X + GOAL_W, GOAL_Y);
    g.lineTo(GOAL_X + GOAL_W + 6, GOAL_Y - 10);
    g.strokePath();
    // Top bar perspective
    g.lineStyle(1.5, 0xcccccc, 0.4);
    g.beginPath();
    g.moveTo(GOAL_X - 6, GOAL_Y - 10);
    g.lineTo(GOAL_X + GOAL_W + 6, GOAL_Y - 10);
    g.strokePath();

    // Inner edge highlight
    g.fillStyle(0xdddddd, 0.4);
    g.fillRect(GOAL_X + POST_W - 1, GOAL_Y + CBAR_H, 1, GOAL_H - CBAR_H);
    g.fillRect(GOAL_X + GOAL_W - POST_W, GOAL_Y + CBAR_H, 1, GOAL_H - CBAR_H);
  },
  ...
  _redrawGK: function(pose) {
    var p = this.gkParts;
    var gkKit = 0x00aa55;  // GK always wears green
    var skin  = 0xf5cba7;
  ...
  _redrawKicker: function(pose) {
    ...
    if (pose === 'idle' || pose === 'standing') {
      ...
      p.bootL.fillStyle(0x111111); p.bootL.fillRect(-13, 42, 13, 7);
      p.bootR.fillStyle(0x111111); p.bootR.fillRect(0, 42, 13, 7);
      p.armL.fillStyle(kit); p.armL.fillRect(-28, -26, 10, 9);
      ...
      p.armL.fillStyle(kit); p.armL.fillRect(-16, -4, 10, 9);
  ...
  _buildBall: function() {
    ...
    this.ballTrailGfx = this.add.graphics().setDepth(8);
  ...
  _redrawBall: function() {
    ...
    // Trail (power shot motion blur)
    this.ballTrailGfx.clear();
    if (this.ballInFlight && this.flightTech === 'power' && this.ballTrail.length > 0) {
      for (var ti = 0; ti < this.ballTrail.length; ti++) {
        var t = this.ballTrail[ti];
        var ta = 0.15 * (ti + 1) / this.ballTrail.length;
        this.ballTrailGfx.fillStyle(0xffffff, ta);
        this.ballTrailGfx.fillCircle(t.x, t.y, r * 0.7);
      }
    }
    // Shadow
  ...
  _buildScoreTracker: function() {
    ...
    this.scoreBg.fillStyle(0x000000, 0.78);
    this.scoreBg.fillRoundedRect(4, baseY, W - 8, SCORE_H, 6);
    this.scoreBg.lineStyle(1, 0x334455, 0.5);
    this.scoreBg.strokeRoundedRect(4, baseY, W - 8, SCORE_H, 6);

    this.scoreHomeName = this.add.text(14, baseY + 4, '', {
      fontFamily:'monospace', fontSize:'10px', color:'#ffffff', fontStyle:'bold'
    }).setDepth(D);
    this.scoreAwayName = this.add.text(W - 14, baseY + 4, '', {
      fontFamily:'monospace', fontSize:'10px', color:'#ffffff', fontStyle:'bold'
    }).setOrigin(1, 0).setDepth(D);

    this.scoreTxt = this.add.text(W / 2, baseY + 2, '0 – 0', {
      fontFamily:'monospace', fontSize:'18px', color:'#fbbf24', fontStyle:'bold'
    }).setOrigin(0.5, 0).setDepth(D);

    this.roundTxt = this.add.text(W / 2, baseY + 24, 'ROUND 1', {
      fontFamily:'monospace', fontSize:'8px', color:'#aaaaaa', letterSpacing:2
    }).setOrigin(0.5, 0).setDepth(D);
    ...
    for (var i = 0; i < 5; i++) {
      ...
      hg.fillStyle(0x555555); hg.fillCircle(18 + i * 20, dotY, 6);
      this.homeDotGfx.push(hg);

      var ag = this.add.graphics().setDepth(D);
      ag.fillStyle(0x555555); ag.fillCircle(W - 18 - i * 20, dotY, 6);
  ...
  _refreshScoreTracker: function() {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      if (i < hKicks.length) {
        ...
        self.homeDotGfx[i].fillCircle(18 + i * 20, dotY, 6);
        if (hOut !== 'goal') {
          self.homeDotGfx[i].lineStyle(2, 0xffffff, 0.8);
          self.homeDotGfx[i].beginPath();
          self.homeDotGfx[i].moveTo(14 + i * 20, dotY - 4); self.homeDotGfx[i].lineTo(22 + i * 20, dotY + 4);
          self.homeDotGfx[i].moveTo(22 + i * 20, dotY - 4); self.homeDotGfx[i].lineTo(14 + i * 20, dotY + 4);
          self.homeDotGfx[i].strokePath();
        }
      } else {
        self.homeDotGfx[i].lineStyle(1, 0x555555);
        self.homeDotGfx[i].strokeCircle(18 + i * 20, dotY, 6);
      }
      self.awayDotGfx[i].clear();
      if (i < aKicks.length) {
        var aOut = aKicks[i].outcome;
        self.awayDotGfx[i].fillStyle(aOut === 'goal' ? 0xfbbf24 : 0xef4444);
        self.awayDotGfx[i].fillCircle(W - 18 - i * 20, dotY, 6);
        if (aOut !== 'goal') {
          self.awayDotGfx[i].lineStyle(2, 0xffffff, 0.8);
          self.awayDotGfx[i].beginPath();
          self.awayDotGfx[i].moveTo(W - 22 - i * 20, dotY - 4); self.awayDotGfx[i].lineTo(W - 14 - i * 20, dotY + 4);
          self.awayDotGfx[i].moveTo(W - 14 - i * 20, dotY - 4); self.awayDotGfx[i].lineTo(W - 22 - i * 20, dotY + 4);
          self.awayDotGfx[i].strokePath();
        }
      } else {
        self.awayDotGfx[i].lineStyle(1, 0x555555);
        self.awayDotGfx[i].strokeCircle(W - 18 - i * 20, dotY, 6);
      }
    }
  },
  ...
  _buildTurnBanner: function() {
    ...
    this.turnBannerBg.fillRoundedRect(W / 2 - 115, SCORE_TOP + SCORE_H + 4, 230, 24, 6);
    this.turnBannerTxt = this.add.text(W / 2, SCORE_TOP + SCORE_H + 16, '', {
  ...
  _buildTechniqueButtons: function() {
    ...
    var BW = 310, BH = 48, BX = (W - BW) / 2, startY = CTRL_Y + 6;
    var BGAP = 6;

    var self = this;
    btnData.forEach(function(bd, i) {
      var by = startY + i * (BH + BGAP);
      ...
      bg.on('pointerover', function(){ bg.clear(); bg.fillStyle(0x2a3a4a, 0.95); bg.fillRoundedRect(BX, by, BW, BH, 8); bg.lineStyle(2, 0x5a8abc, 1); bg.strokeRoundedRect(BX, by, BW, BH, 8); });
      bg.on('pointerout', function(){ bg.clear(); bg.fillStyle(0x1a2a3a, 0.92); bg.fillRoundedRect(BX, by, BW, BH, 8); bg.lineStyle(1.5, 0x3a5a7a, 0.7); bg.strokeRoundedRect(BX, by, BW, BH, 8); });
      bg.on('pointerdown', function(){ self._onTechniqueSelect(bd.key); });

      var mainTxt = self.add.text(BX + 14, by + 8, bd.label, {
        fontFamily:'monospace', fontSize:'14px', color:'#ffffff', fontStyle:'bold'
      });
      var subTxt = self.add.text(BX + 14, by + 28, bd.sub, {
  ...
  // ── POWER BAR — colors vary by shot type ─────────────────────────
  _buildPowerBarUI: function() {
    var D = 10;
    this.powerBarContainer = this.add.container(0, 0).setDepth(D).setAlpha(0);

    var pbBarX = W - 52, pbBarY = GOAL_Y + 10, pbBarW = 24, pbBarH = 180;
    this.pbX = pbBarX; this.pbY = pbBarY; this.pbW = pbBarW; this.pbH = pbBarH;

    var bg = this.add.graphics();
    bg.fillStyle(0x000000, 0.7); bg.fillRoundedRect(pbBarX - 4, pbBarY - 22, pbBarW + 8, pbBarH + 42, 8);
    bg.lineStyle(1, 0x334455); bg.strokeRoundedRect(pbBarX - 4, pbBarY - 22, pbBarW + 8, pbBarH + 42, 8);
    bg.fillStyle(0x222222); bg.fillRect(pbBarX, pbBarY, pbBarW, pbBarH);
    bg.lineStyle(1, 0x444444); bg.strokeRect(pbBarX, pbBarY, pbBarW, pbBarH);
    bg.lineStyle(1, 0xffffff, 0.3);
    [25, 50, 75].forEach(function(pct){
      var ty = pbBarY + pbBarH - (pbBarH * pct / 100);
      bg.beginPath(); bg.moveTo(pbBarX, ty); bg.lineTo(pbBarX + pbBarW, ty); bg.strokePath();
    });

    this.pbLabelTxt = this.add.text(pbBarX + pbBarW / 2, pbBarY - 14, 'POWER', {
      fontFamily:'monospace', fontSize:'8px', color:'#aaaaaa', fontStyle:'bold', letterSpacing:1
    }).setOrigin(0.5, 1);
    this.pbValueTxt = this.add.text(pbBarX + pbBarW / 2, pbBarY + pbBarH + 6, '0%', {
  ...
  _updatePowerBarFill: function() {
    ...
    // Color varies by shot technique
    var color;
    var tech = this.pendingTech;
    if (tech === 'power') {
      color = pct < 80 ? 0xef4444 : pct < 90 ? 0xfbbf24 : pct <= 98 ? 0x4ade80 : 0xef4444;
    } else if (tech === 'panenka') {
      color = pct < 25 ? 0xef4444 : pct < 35 ? 0xfbbf24 : pct <= 45 ? 0x4ade80 : 0xef4444;
    } else {
      // regular
      color = pct < 60 ? 0xef4444 : pct < 75 ? 0xfbbf24 : pct <= 90 ? 0x4ade80
            : pct <= 95 ? 0xfbbf24 : 0xef4444;
    }
    ...
    var txtColor = color === 0x4ade80 ? '#4ade80' : color === 0xfbbf24 ? '#fbbf24' : '#ef4444';
    this.pbValueTxt.setStyle({ color: txtColor });
  },

  // ── ACCURACY RING — positioned near shoot button area ─────────────
  _buildAccuracyRingUI: function() {
    var D = 10;
    this.ringGfx = this.add.graphics().setDepth(D).setAlpha(0);
    this.ringHintTxt = this.add.text(W * 0.75, H * 0.74, 'TAP ANYWHERE', {
  ...
  _drawRing: function() {
    ...
    var cx = Math.round(W * 0.75), cy = Math.round(H * 0.82);
    // Color: small=green (accurate), large=red (inaccurate)
    var frac = (r - RING_MIN) / (RING_MAX - RING_MIN);
    var red  = Math.round(Phaser.Math.Linear(0x4a, 0xef, frac));
    var grn  = Math.round(Phaser.Math.Linear(0xde, 0x44, frac));
  ...
  _enterPhase: function(phase) {
    ...
    if (phase === 'technique_select') {
      ...
      var baseDur = 900 + skill * 13;
      // Speed varies by shot type
      var cycleDur;
      if (this.pendingTech === 'power') cycleDur = baseDur / 1.8;
      else if (this.pendingTech === 'panenka') cycleDur = baseDur * 1.6;
      else cycleDur = baseDur;
  ...
  // ─────────────────────────────────────────────────────────────────
  // INPUT HANDLERS — aim width varies by technique
  // ─────────────────────────────────────────────────────────────────
  _onPointerMove: function(p) {
    if (this.phase !== 'aiming') return;
    // Aim width varies by shot type
    var aimW = this.pendingTech === 'panenka' ? W * 0.25
             : this.pendingTech === 'power'   ? W * 0.45
             : INNER_W + 50;
    var halfAim = aimW / 2;
    this.aimX = Phaser.Math.Clamp(p.x, W / 2 - halfAim, W / 2 + halfAim);
    this.aimY = Phaser.Math.Clamp(p.y, INNER_Y - 25, INNER_Y + INNER_H + 25);
    this._redrawAimLine();
  },
  ...
  _onAccuracyTap: function() {
    ...
    var r = this.ringData.radius;
    // Accuracy mapping: small ring = high accuracy
    var acc;
    if (r <= 15)      acc = 95 + Math.round((15 - r) / 15 * 5);
    else if (r <= 25) acc = 70 + Math.round((25 - r) / 10 * 24);
    else if (r <= 35) acc = 40 + Math.round((35 - r) / 10 * 29);
    else              acc = Math.max(0, Math.round((45 - r) / 10 * 39));
  ...
  // ─────────────────────────────────────────────────────────────────
  // KICK RESOLUTION — with pre-shot tension
  // ─────────────────────────────────────────────────────────────────
  _resolveUserKick: function(zone, power, accuracy, technique, gkDive) {
    ...
    // ── Pre-shot tension: 1.5s pause ──
    var tensionOverlay = this.add.graphics().setDepth(11);
    tensionOverlay.fillStyle(0x000000, 0.15);
    tensionOverlay.fillRect(0, 0, W, H);

    var dots = this.add.text(W / 2, H * 0.5, '...', {
      fontFamily:'monospace', fontSize:'48px', color:'#ffffff', fontStyle:'bold'
    }).setOrigin(0.5).setDepth(11).setScale(0.3);
    this.tweens.add({ targets: dots, scaleX:1, scaleY:1, duration:1500, ease:'Power2.easeOut' });

    // GK quick shuffle anticipation
    if (this.gkIdleTween) { this.gkIdleTween.stop(); this.gkIdleTween = null; }
    this.tweens.add({
      targets: this.gkContainer,
      x: GK_X - 15, duration:150, yoyo:true, repeat:4, ease:'Sine.easeInOut'
    });

    this.time.delayedCall(1500, function() {
      tensionOverlay.destroy();
      dots.destroy();
      self._executeKick(zone, technique, gkDive, outcome);
    });
  },
  ...
  _executeKick: function(zone, technique, gkDive, outcome) {
    ...
    this._animateBall(target.x, target.y, technique, outcome, function(){
      ...
      self.ballTrail = [];
      self.ballTrailGfx.clear();
      self._onBallArrived(zone, technique, outcome);
    });
  ...
  _onBallArrived: function(zone, technique, outcome) {
    var self = this;
    ...
    this._refreshScoreTracker();
    this._showCommentary(pickLine(outcome, technique));
  ...
  // ─────────────────────────────────────────────────────────────────
  // ANIMATIONS — ball trajectory varies by technique
  // ─────────────────────────────────────────────────────────────────
  _animateBall: function(tx, ty, technique, outcome, onComplete) {
    var self = this;
    // Duration and arc height per technique
    var dur, arcH;
    if (technique === 'panenka')     { dur = 800; arcH = H * 0.25; }
    else if (technique === 'power')  { dur = 300; arcH = H * 0.04; }
    else                             { dur = 500; arcH = H * 0.10; }
    ...
    this.tweens.add({
      ...
      onUpdate: function(){
        ...
        var s  = 1 - t * 0.45;
        // Store trail for power shot
        if (technique === 'power') {
          self.ballTrail.push({x:self.ballPos.x, y:self.ballPos.y});
          if (self.ballTrail.length > 3) self.ballTrail.shift();
        }
